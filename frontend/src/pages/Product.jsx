...
const Product = () => {
  ...
  const { products, currency, addToCart, wishlist, addToWishlist, removeFromWishlist, token } = useContext(ShopContext);
  ...
  const handleWishlistClick = (e) => {
    e.preventDefault();
    if (!token) {
      addToWishlist(productId); // Will show login toast
      return;
    }
    if (isInWishlist) {
    ...
    <div className='border-t-2 pt-10 transition-opacity ease-in duration-500 opacity-100'>
      ...
      <div className='flex gap-12 sm:gap-12 flex-col sm:flex-row'>
        ...
        <div className='flex-1'>
          <h1 className='font-medium text-2xl mt-2 flex items-center gap-2'>
            {productData.name}
            <button
